...
import { VictoryBar, VictoryChart, VictoryGroup, VictoryVoronoiTooltip, VictoryAxis } from 'victory';
...
const propTypes = {
  ...
  customizedTooltip: PropTypes.bool,
...
export default class BarChartAxis extends React.Component {

  static renderTooltip(tooltip, dateFormat, unit, tooltipIsBoolean, customizedTooltip, tooltipBooleanTrueLabel, tooltipBooleanFalseLabel) {
    if (tooltip) {
      if (tooltipIsBoolean) {
        return (
      ...
      if (customizedTooltip) {
        return (
          <VictoryVoronoiTooltip labels={d => ` ${Math.round(d.y * 100) / 100}${unit}`} />
        );
      }
      return (
      ...
      <div className="chart-container" {...attributes}>
        ...
        >
          <VictoryGroup data={data} x={x} y={y} >
            ...
            />
            {BarChartAxis.renderTooltip(tooltip, dateFormat, unit, tooltipIsBoolean, customizedTooltip,
              tooltipBooleanTrueLabel, tooltipBooleanFalseLabel)}
          </VictoryGroup>
          <VictoryAxis tickCount={xTickCount} label="Time" />
          {BarChartAxis.renderYAxis(yTickCount, tickFormat)}
...
BarChartAxis.propTypes = propTypes;
BarChartAxis.defaultProps = defaultProps;
